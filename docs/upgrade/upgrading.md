...
Starting with this version, the EDA operator **no longer deploys or uses Redis**. EDA now uses [dispatcherd](https://github.com/ansible/dispatcherd), a PostgreSQL `pg_notify`-based task queuing system that eliminates the need for a separate Redis instance.
...
- The `redis_image` and `redis_image_version` CR fields are deprecated and ignored.
