...
> **Important:** Starting with this version of the EDA operator, Redis is **no longer deployed or used**.
...
| `spec.redis` and `spec.redis.redis_secret`  | Deprecated in the CRD; ignored if still present              |
| `redis_image` / `redis_image_version`       | Deprecated in the CRD; ignored if still present              |
...
A: No. The `spec.redis` CRD fields are deprecated and ignored by the operator. If they are still present in your CR (e.g. from a previous version), the operator will ignore them and log a deprecation warning. The fields will be removed from the CRD schema in a future `apiVersion`.
